...
CXXFLAGS := $(INCLUDES) -g -O0 -Wall -Wextra -Wconversion -Wsign-conversion -Werror \
			-Wno-unused-parameter -std=c++11 -pedantic-errors -fdiagnostics-color=always --coverage
...
tests: $(TEST_OBJS) $(OBJS_NO_MAIN) # linka os objetos dos testes com todos os objetos exceto o main.o
	$(CXX) $^ -o $(TEST_DIR)/$(TARGET_TESTS) $(LDFLAGS)
...
coverage: run_tests
	gcovr -r . --exclude="$(TEST_DIR)/doctest.h" --exclude="thirdParty/" --exclude="build/"
